...
export const removeElementAtom: Atom = {
  name: 'removeElement',
  version: '1.0.0',
  meta: {
    input: [
...
export const closestAtom: Atom = {
  name: 'closest',
  version: '1.0.0',
  meta: {
    input: [
      { name: 'id', type: 'string', description: 'Element ID' },
      { name: 'selector', type: 'string', description: 'CSS selector to match ancestor' }
    ],
  ...
  execute: async (input: { id: string; selector: string }, context: Context): Promise<Result> => {
    ...
    const closestEl = el.closest(input.selector);
    if (!closestEl) {
      return { success: true, data: null };
    }

    const closestId = (closestEl as HTMLElement).id || generateElementId();
    if (!(closestEl as HTMLElement).id) {
      (closestEl as HTMLElement).id = closestId;
    }
    setElement(closestId, closestEl as HTMLElement);
    
    return { success: true, data: closestId };
  },
};
...
export const screenshotAtom: Atom = {
  ...
  meta: {
    input: [
      { name: 'selector', type: 'string', description: 'CSS selector or element ID to capture, "body" for full page' }
    ],
  ...
  execute: async (input: { selector: string }, context: Context): Promise<Result> => {
    ...
    const ctx = getCanvasContext();
    if (!canvas || !ctx) {
      return { success: false, error: 'Canvas not initialized, call setCanvas first' };
    }

    let targetEl: HTMLElement | null = null;

    if (input.selector === 'body' || !input.selector) {
      targetEl = document.body;
    } else {
      const fromStore = getElement(input.selector);
      if (fromStore) {
        targetEl = fromStore;
      } else {
        targetEl = document.querySelector(input.selector);
      }
    }

    if (!targetEl) {
      return { success: false, error: `Element "${input.selector}" not found` };
    }
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="${rect.width}" height="${rect.height}">
          <foreignObject width="100%" height="100%">
            <div xmlns="http://www.w3.org/1999/xhtml">
              ${new XMLSerializer().serializeToString(targetEl)}
            </div>
          </foreignObject>
        </svg>`;
...
export const canvasToDataURLAtom: Atom = {
  name: 'canvasToDataURL',
  version: '1.0.0',
  meta: {
    input: [
      { name: 'format', type: 'string', description: 'Image format: "image/png" or "image/jpeg"' }
    ],
