...
interface AcsNode {
  ...
  arrayRef?: string;
  maxItems?: number;
  inlineCondAtom?: { atom: string; params: Record<string, any> };
}
...
export class AcsParser {
  ...
  private parseIf(): AcsNode {
    ...
    const firstIdent = this.readIdentifier();
    this.skipWhitespaceAndComments();

    let ref: string;
    let inlineCondAtom: { atom: string; params: Record<string, any> } | undefined;

    if (this.peek('(')) {
      const params = this.parseParams();
      inlineCondAtom = { atom: firstIdent, params };
      ref = `__inline_cond_${savedPos}`;
    } else {
      ref = firstIdent;
    }
    ...
    const ifNode: AcsNode = { type: 'if', ref, body, elseBody };
    if (inlineCondAtom) {
      ifNode.inlineCondAtom = inlineCondAtom;
    }
    return ifNode;
  }
  ...
  private compileNodes(acsNodes: AcsNode[], graph: Graph): void {
    for (const node of acsNodes) {
      switch (node.type) {
        ...
        case 'if': {
          if (node.inlineCondAtom) {
            const condNodeId = node.ref!;
            const condGraphNode: GraphNode = { atom: node.inlineCondAtom.atom, ...node.inlineCondAtom.params };
            graph.order.push(condNodeId);
            graph.nodes[condNodeId] = condGraphNode;
          }
...
export class AcsDecompiler {
  decompile(graph: Graph, name: string = 'unnamed'): string {
    ...
    for (const nodeId of graph.order) {
      ...
      if (nodeId.startsWith('__inline_cond_') && node.atom !== 'branch') continue;
      lines.push(...this.decompileNode(nodeId, node, 1, graph.nodes));
    }
  ...
  private decompileNode(nodeId: string, node: GraphNode, indent: number, allNodes?: Record<string, GraphNode>): string[] {
    ...
    if (node.atom === 'branch') {
      ...
      let condStr = condRef || nodeId;
      if (condRef && condRef.startsWith('__inline_cond_') && allNodes && allNodes[condRef]) {
        const condNode = allNodes[condRef];
        const condParams = this.formatParams(condNode);
        condStr = `${condNode.atom}(${condParams})`;
        lines.push(`${ifPrefix}if ${condStr} {`);
      } else {
        lines.push(`${ifPrefix}if ${condStr} {`);
      }
  ...
  private decompileSubGraph(graph: Graph, indent: number): string[] {
    const lines: string[] = [];
    for (const nodeId of graph.order) {
      if (nodeId.startsWith('__inline_cond_') && graph.nodes[nodeId].atom !== 'branch') continue;
      lines.push(...this.decompileNode(nodeId, graph.nodes[nodeId], indent, graph.nodes));
    }
    return lines;
  }
